...
async function fetchLatestTyphoonEFF() {
	try {
		if (!KMA_KEY) {
			if (VERBOSE)
				console.warn('[typhoon][eff] KMA_API_KEY missing, fallback 4')
			return 4
		}
...
exports.ingestAllBuildingsWeather = async () => {
	...
	for await (const b of cursor) {
		...
		try {
			...
			if (!coord) {
				skip++
				console.warn('[weather][skip][geocode-null]', {
					building: bid,
					name,
					addr,
				})
			...
			fail++
			console.error('[weather][fail][geocode]', {
				building: bid,
				name,
				addr,
				msg: e.message,
		...
		try {
			...
			fail++
			console.error('[weather][fail][owm]', {
				building: bid,
				name,
				msg: e.message,
			})
